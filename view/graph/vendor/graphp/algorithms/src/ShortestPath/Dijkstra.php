...
{
    public $totalCostOfCheapestPathTo = [];
    public $predecesVertexOfCheapestPathTo = [];
    /**
    ...
    {
        if (count($this->predecesVertexOfCheapestPathTo) == 0) {
            $this->calculate();
        }
        // algorithm is done, return resulting edges
        return $this->getEdgesCheapestPredecesor($this->predecesVertexOfCheapestPathTo);
    }
    public function calculate() {
            // start node distance
            $this->totalCostOfCheapestPathTo[$this->vertex->getId()] = INF;
            ...
            // predecessor
            $this->predecesVertexOfCheapestPathTo[$this->vertex->getId()] = $this->vertex;
            ...
            for ($i = 0; $i < $totalCountOfVertices; ++$i) {
                ...
                foreach ($currentVertex->getEdgesOut() as $edge) {
                    ...
                    if (!isset($usedVertices[$targetVertexId])) {
                        ...
                        $newCostsToTargetVertex = $this->totalCostOfCheapestPathTo[$currentVertexId] + $weight;
                        if (is_infinite($newCostsToTargetVertex)) {
                            $newCostsToTargetVertex = $weight;
                        }

                        if ((!isset($this->predecesVertexOfCheapestPathTo[$targetVertexId]))
                                // is the new path cheaper?
                                || $this->totalCostOfCheapestPathTo[$targetVertexId] > $newCostsToTargetVertex){
                                    ...
                                    $this->predecesVertexOfCheapestPathTo[$targetVertexId] = $currentVertex;
                        }
                    }
                }
            }

            if ($this->totalCostOfCheapestPathTo[$this->vertex->getId()] === INF) {
                unset($this->predecesVertexOfCheapestPathTo[$this->vertex->getId()]);
            }
    }
}
