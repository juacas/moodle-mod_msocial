...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildDateRangePicker($originalElement, instance, options) {
		...
		function formatRangeForDisplay(range) {
			...
			var starttext = range.start.getTime() <= -8639999992800000 ? "-∞" : $.datepicker.formatDate(dateFormat, range.start) ;
			var endtext = range.end.getTime() >= 8639999992800000 ? "∞" : $.datepicker.formatDate(dateFormat, range.end);
			return starttext + (+range.end !== +range.start ? options.rangeSplitter + endtext : '');
		}
